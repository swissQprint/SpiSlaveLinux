...
static inline void mcspi_slave_write_reg(void __iomem *base, u32 idx, u32 val) {
...
static void mcspi_slave_pio_rx_transfer(unsigned long data) {
    ...
    if(mcspi_slave_bytes_per_word(slave->bits_per_word) == 1) {
        ...
        do {
            c -= 1;
            if(mcspi_slave_wait_for_bit(chstat, MCSPI_CHSTAT_RXS) < 0)
                goto out;
...
static void mcspi_slave_pio_tx_transfer(struct spi_slave *slave, unsigned int length) {
    unsigned int c;
    void __iomem *tx_reg;
    void __iomem *chstat;
    ...
    if(mcspi_slave_bytes_per_word(slave->bits_per_word) == 1) {
        ...
        do {
            c -= 1;
            if(mcspi_slave_wait_for_bit(chstat, MCSPI_CHSTAT_TXS) < 0)
                goto out;
...
static int mcspi_slave_set_irq(struct spi_slave *slave) {
    ...
    ret = devm_request_irq(slave->dev, slave->irq, (irq_handler_t)mcspi_slave_irq, IRQF_TRIGGER_NONE, DRIVER_NAME, slave);
    if(ret) {
...
static void mcspi_slave_dma_request_enable(struct spi_slave *slave, unsigned int rw) {
    u32 l;
...
static void mcspi_slave_dma_request_disable(struct spi_slave *slave, unsigned int rw) {
    u32 l;
...
static void mcspi_slave_dma_rx_callback(void *data) {
    ...
    dma_sync_single_for_cpu(slave->dev, slave->dma_channel.rx_dma_addr, slave->len, DMA_FROM_DEVICE);
...
static int mcspi_slave_dma_tx_transfer(struct spi_slave *slave) {
    ...
    dma_sync_single_for_device(slave->dev, dma_channel->tx_dma_addr, slave->len, DMA_TO_DEVICE);


    tx_desc = dmaengine_prep_slave_sg(dma_channel->dma_tx, &dma_channel->sg_tx, 1, DMA_MEM_TO_DEV, DMA_PREP_INTERRUPT | DMA_CTRL_ACK);
...
static int mcspi_slave_dma_rx_transfer(struct spi_slave *slave) {
    ...
    rx_desc = dmaengine_prep_slave_sg(dma_channel->dma_rx, &dma_channel->sg_rx, 1, DMA_DEV_TO_MEM, DMA_PREP_INTERRUPT | DMA_CTRL_ACK);
...
static int mcspi_slave_setup_dma_transfer(struct spi_slave *slave, int do_mapping) {
    ...
    if(do_mapping && dma_channel->dma_tx && tx_buf != NULL) {
        ...
        dma_channel->tx_dma_addr = dma_map_single(slave->dev, (void *)tx_buf, slave->len, DMA_TO_DEVICE);
    ...
    if(do_mapping && dma_channel->dma_rx && rx_buf != NULL) {
        ...
        dma_channel->rx_dma_addr = dma_map_single(slave->dev, (void *)rx_buf, slave->len, DMA_FROM_DEVICE);
...
static int mcspi_slave_clr_transfer(struct spi_slave *slave) {
    ...
    dma_unmap_single(slave->dev, slave->dma_channel.tx_dma_addr, slave->len, DMA_TO_DEVICE);

    dma_unmap_single(slave->dev, slave->dma_channel.rx_dma_addr, slave->len, DMA_FROM_DEVICE);
...
static int mcspi_slave_setup_on_probe(struct spi_slave *slave) {
    ...
    if(mcspi_slave_wait_for_bit(slave->base + MCSPI_SYSSTATUS, MCSPI_SYSSTATUS_RESETDONE) == 0) {
        ...
        if(SPI_TRANSFER_MODE == SPI_DMA_MODE && (slave->dma_channel.dma_rx == NULL || slave->dma_channel.dma_tx == NULL)) {
...
static int mcspi_slave_probe(struct platform_device *pdev) {
    ...
    if(minor < N_SPI_MINORS) {
        ...
        dev = device_create(spislave_class, slave->dev, slave->devt, slave, "spislave%d", slave->bus_num);
...
static ssize_t spislave_read(struct file *flip, char __user *buf, size_t count, loff_t *f_pos) {
    struct spi_slave *slave;
    int error_count = 0;
    ...
    pr_debug("%s: read end count:%d rx_offset:%d\n", DRIVER_NAME, error_count, slave->rx_offset);
...
static ssize_t spislave_write(struct file *flip, const char __user *buf, size_t count, loff_t *f_pos) {
    ssize_t ret = 0;
    struct spi_slave *slave;
...
static long spislave_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
    int ret = 0;
    int err = 0;
    struct spi_slave *slave;
...
static unsigned int spislave_event_poll(struct file *filp, struct poll_table_struct *wait) {
    struct spi_slave *slave;
